...
public class MockableGenerator : IIncrementalGenerator
{
	...
	private static string GenerateWrapperClass(TypeModel model)
	{
		...
		builder.AppendLine("using DynaMock;"); // Add using for IMockProvider
		...
		// Add constructor with IMockProvider injection
		builder.AppendLine($"        public {className}({model.Name}{genericTypeParams} realImplementation, IMockProvider<{model.Name}{genericTypeParams}>? mockProvider = null)");
	...
	private static void GenerateMethod(StringBuilder builder, MethodModel method, bool isInterface)
	{
		...
		//string awaitPrefix = method.IsAsync ? "await " : ""; // No need for this
		string awaitPrefix = "";
	...
	private static void GenerateEvent(StringBuilder builder, EventModel evt, bool isInterface)
	{
		...
		builder.AppendLine("        {");
		builder.AppendLine($"            add => RealImplementation.{evt.Name} += value;");
		builder.AppendLine($"            remove => RealImplementation.{evt.Name} -= value;");
		builder.AppendLine("        }");
		builder.AppendLine();
	}
}
